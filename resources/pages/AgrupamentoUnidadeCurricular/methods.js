...
const Methods = () => {
    ...
    const [courseUnitGroup, setCourseUnitGroup] = useState();
    ...
    const isFormValid = (methodList) => {
        let isValid = methodList.length > 0
            && methodList.filter((x) => x).length === epochs.length;
    ...
    const loadMethods = () => {
        ...
        setMethods([]);
        axios.get(`/course-units/${paramsId}/methods`).then((res) => {
    ...
    useEffect(() => {
        axios.get(`/course-units/${paramsId}/epochs`).then((res) => {
            setIsLoading(false);
            if (res.status === 200) {
                setCourseUnitGroup(res.data.courseUnitGroup);
    ...
    return (
        <Container>
            ...
            <Card fluid>
                <Card.Content
                    header={`Métodos de avaliação para o Agrupamento de Unidades Curriculares: ${courseUnitGroup || ''}`}/>
                {isLoading && (
