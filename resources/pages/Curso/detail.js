...
const Detail = ({ match }) => {
  ...
  const loadCourseDetail = () => {
    ...
    axios.get(`/courses/${match.params?.id}`).then((res) => {
      setLoading(false);
  ...
  const setCoordinator = () => {
    axios.patch(`/courses/${match.params.id}/coordinator`, {
  ...
  const removeStudent = (studentId) => {
    axios.delete(`/courses/${match.params.id}/student/${studentId}`).then((res) => {
      if (res.status === 200) {
  ...
  const onSaveCourse = ({
    ...
    axios.patch(`/courses/${match.params.id}`, {
      code,
      name,
      initials,
      degree: level,
  ...
  const addStudent = () => {
    ...
    axios.patch(`/courses/${match.params.id}/student`, {
              ...
              <Card.Content>
                <Form>
                  ...
                  <Form.Group widths="2">
                    ...
                    <Form.Button disabled={loading || !hasPermissionToDefineCoordinator} label="Guardar?" onClick={setCoordinator} color="green" icon labelPosition="left">
                      <Icon name="save" />
                      Guardar coordenador
                    </Form.Button>
                  </Form.Group>
              ...
              <Card.Content extra>
                <Grid padded="vertically">
                  <Grid.Column width="8">
                    <Grid.Row>
                      <Header>Ramos</Header>
                      <Button color="green" onClick={() => setAdditionalBranches((current) => [...current, { order: current.length }])}>Adicionar ramo</Button>
                    </Grid.Row>
                    <br />
                    <Grid.Row>
                      <List>
                          ...
                          <List.Item key={branch.id}>
                            ...
                            <List.Content>
                              <Form.Input
                                ...
                                onChange={(e, { value }) => setAdditionalBranches((current) => [...current.filter((x) => x.order !== branch.order), { order: branch.order, name: value }])}
                              />
                            </List.Content>
                          </List.Item>
                        ))}
