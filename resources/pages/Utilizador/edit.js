...
const New = ({match}) => {
    ...
    const initialValues = useMemo(() => {
        ...
        return {id, email, name, enabled, groups: groups?.map(({id}) => id)};
    }, [userDetail]);
        ...
        <Container style={{marginTop: '2em'}}>
            <FinalForm
                ...
                render={({handleSubmit}) => (
                    <Form>
                        <Card fluid>
                            <Card.Content header="Editar Utilizador"/>
                            <Card.Content>
                                <Form.Group widths="equal">
                                    <Form.Input label="Nome" value={initialValues.name} disabled/>
                                    <Form.Input label="Email" value={initialValues.email} disabled/>
                                </Form.Group>
                                <Field name="groups">
                                    {({input: groupsInput}) => (
                                        <Form.Dropdown
                                ...
                                <Field name="enabled" type="checkbox">
                                    {({input: enabledUserInput}) => (
                                        <Checkbox label="Utilizador ativo?" toggle
