...
class TipoInterrupcaoRequest extends FormRequest
{
    /**
    ...
    public function rules(){
        $rules =  [
            'nome'                                  => 'string|max:120|unique:tipos_interrupcoes,nome,'.$this->id,
            'descricao'                             => 'string|max:255|unique:tipos_interrupcoes,descricao,'.$this->id,
        ...
        for ($i = 0; $i < count($this->traducoes); $i++) {
            if($this->update) {
                $rules['id']                            = 'required|integer|exists:tipos_interrupcoes,id';
